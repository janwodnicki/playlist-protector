...
def _WriteProcessOutputToFileAndPrintInstanceStatus(
  ...
  for i, instance_process in enumerate(instance_processes):
    completed = i + 1
    # subprocess.out.read() waits for the process to finish.
    instance_process.out_content = instance_process.out.read().decode()
...
def main():
  ...
  try:
    ...
    print("ERROR:\n%s" % e)
    print(f"See script log file: {script_log_file}")
    return
  instance_processes = []
